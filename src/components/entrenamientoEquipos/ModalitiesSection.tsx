...
// Los datos de las modalidades se definen dentro del mismo archivo.
const modalities = [
    {
...
export function ModalitiesSection() {
    return (
        // Contenedor de la sección con la imagen de fondo y el efecto parallax
        <section
            className="relative w-full py-20 sm:py-28 bg-cover bg-center bg-fixed"
            style={{ backgroundImage: "url('/images/equipos/modalitySection.jpg')" }}
        >
            {/* Capa de superposición oscura para la legibilidad del texto */}
            <div className="absolute inset-0 bg-black/60 z-0" />

            {/* Contenedor del contenido, debe estar por encima de la superposición */}
            <div className="relative z-10 mx-auto max-w-7xl px-6">
                ...
                <div className="mt-10 grid grid-cols-1 gap-8 max-w-5xl mx-auto md:grid-cols-2">
                    {modalities.map((modality) => (
                        <div
                            key={modality.title}
                            className="rounded-2xl border border-white/20 bg-white/4 p-8 shadow-lg backdrop-blur-lg transition-all duration-300 hover:bg-white/10 hover:scale-[1.02]"
                        >
